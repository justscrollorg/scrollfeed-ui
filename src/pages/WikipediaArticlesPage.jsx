...
function WikipediaArticlesPage() {
  ...
  return (
    <>
      <h1 className="text-3xl font-bold text-green-700 mb-6">Wikipedia Articles</h1>
      <div className="flex flex-wrap gap-4">
        {articles.map((article, index) => (
          <div key={index} className="w-full sm:w-1/2 md:w-1/3 lg:w-1/4 bg-white p-4 shadow rounded">
            <h2 className="font-semibold text-lg mb-2">{article.Title}</h2>
            {article.Image && (
              <img src={article.Image} alt={article.Title} className="mb-2 w-full h-40 object-cover rounded" />
            )}
